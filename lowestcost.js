...
function LowestQueue(){
    ...
    this.add = function(cell){
      if(this.queue.length==0){
        ...
        while(i<this.queue.length){
          ...
          if(cell.cost<this.queue[i].cost && found==false && added==false){
            newQueue.push(cell);
            added=true;
    ...
    this.print = function(){
      for (let i =0; i<this.queue.length;i++){
        console.log("x: "+this.queue[i].x+" y: "+this.queue[i].y+" cost: "+this.queue[i].cost);
      }
    }
}
...
let lowestcost = function(){
  finished = false;
  found = false;
  queue = new LowestQueue();
